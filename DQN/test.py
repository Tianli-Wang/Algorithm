...
def select_action(state):
    ...
    # at the begining, pretend to relay on the policy net, and then gradually use the target net.
    steps_done += 1
    if sample > eps_threshold:  # 概率1-ε：选择模型认为的最优动作
        with torch.no_grad():  # 禁用梯度计算（节省资源，因仅用于推理）
            # policy_net(state)：输入状态，输出每个动作的Q值（形状：[1, n_actions]）
            # max(1)：在第1维度（动作维度）取最大值，返回（最大值, 索引）
            # indices.view(1,1)：将最大Q值对应的动作索引转换为形状[1,1]的张量
            # 计算当前状态下所有动作的Q值
            return policy_net(state).max(1).indices.view(1, 1)
    else:
        return torch.tensor([[env.action_space.sample()]], device=device, dtype=torch.long)
        # 从环境的动作空间中随机采样一个动作，torch.tensor(...) ：将随机动作转换为张量并指定设备和数据类型
...
def optimize_model():
    ...
    # 将 “列表 of 转换” 转换为 “转换 of 列表”。例如，原本是[Transition(s1,a1,s1',r1), Transition(s2,a2,s2',r2), ...]，转换后batch.state是所有s的列表，batch.action是所有a的列表，方便批量处理。
