...
class REINFORCE:
    def __init__(self, state_dim, hidden_dim, action_dim, gamma, device,lr):
    ...
    def update(self, transition_dict):
        ...
        for i in reversed(range(len(reward_list))):
            ...
            log_prob = torch.log(self.policy_net(state).gather(1, action)) ## 在列维度上按张量action索引
            G = reward + self.gamma * G
            loss = - G * log_prob ## 负号为了梯度下降
            loss.backward()
